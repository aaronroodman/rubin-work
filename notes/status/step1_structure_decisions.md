...
- **Output** — the path names the data the product depends on:

| study kind | output path | why |
|---|---|---|
| **A. data-dependent, FAM** | `output/<study>/<param_set>/` | the product is only meaningful for one processing run |
| **B. data-dependent, other source** | `output/<study>/<source_id>/` | same rule, different source (e.g. a value-added DB build) |
| **C. data-independent** | `output/<study>/` | depends on the optical prescription or S-matrix, not on any night's data |

A and B are the same rule with a different source; C is the degenerate case where there
is no data source to name. One rule, three outcomes — not three special cases.

### D2. `output/<study>/<param_set>/`, not `output/<param_set>/<study>/`

The current tree uses `output/<param_set>/<study>/`. This inverts.

**Reason:** the axis is ragged. Not every study is repeated for every `param_set`, so
param_set-first creates a sparse matrix in which "not run" is indistinguishable from "not
applicable". Study-first means a study directory lists exactly the param_sets it was
actually run for, which is self-documenting, and kind-C studies fit the same scheme with
no param_set level rather than needing an exception.

**Cost, accepted:** deleting a superseded `param_set` becomes N directory removals
instead of one. Scriptable (`find output -maxdepth 2 -type d -name '<param_set>'`), and
since deletion is a must-ask it is a one-time operation, not daily friction.
...
- **For per-study dirs:** cross-study coupling is almost nil — only 5 study→study imports
  across 67 files.
- **Against, and decisive:** the shared flat modules are heavily used (`aos_state.py` by
  **14** study files, `dz_plotting.py` by 5, `fam_selection.py` and `miw_io.py` by 4
  each). Per-study directories give them no home except a pseudo-study, which is what
...
1. Two incompatible output schemes running at once — five studies wrote to
   `output/<study>/`, seven to `output/<param_set>/<study>/`, and `smatrix_vmode` wrote to
   **both**.
...
| 7 | Invert `output/<param_set>/<study>/` → `output/<study>/<param_set>/` (D2) | output tree + path constants in code | yes |
