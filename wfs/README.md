...
- **wfs_batoid_pupil_compare**: Pure `batoid` for the trace; imports `lsst.obs.lsst` +
  ...
  §9 attributes off-axis vignetting per surface via `batoid.Optic.traceFull`. Needs
  `danish` + `lsst.ts.wep` (`getTaskInstrument`, `DefocalType`).
