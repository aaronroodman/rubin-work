...
- **wfs_batoid_pupil_compare**: Pure `batoid` (no butler, no `batoid_rubin` download).
  Uses the LSST model yaml (`LSST_r.yaml` design, or `Rubin_v3.12_r`/`v3.14_r` as-built)
  shipped with batoid; hexapod pistons via `withGloballyShiftedOptic` on `LSSTCamera`
  and `M2`. Finding: camera-8mm vs camera-4+M2-4 give different donut outer radii
  (~3.45 vs ~3.32 mm at field 1.69°) — the pupil does differ.
