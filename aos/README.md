...
| `intrinsic_camera_telescope_split.ipynb` | Decompose the measured intrinsic Z4-optical map into a telescope-fixed (OCS) component and a camera-fixed (CCS) component that rotates with the rotator, using FAM intrinsic maps built at several rotator angles. Fourier-per-radius least-squares (`A_{d,m}=O_m+C_m e^{-imsθ}`); auto rotation-sign; the degenerate axisymmetric m=0 part assigned by a flag; actual mean rotator read from the fits parquet. Writes O/C maps + a diagnostics PDF. Decomposition core in `code/intrinsic_split.py`. | 2026-06-10 | 2026-06-10 |
