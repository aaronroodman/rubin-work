...
|----------|-------------|---------|---------------|
| `guider_iq.ipynb` | Per-image science FWHM (from ConsDB `psf_sigma_median`) vs. guider metrics over a `day_obs` range, split **by filter band** (one page per band in a multipage PDF). Per band: robust (Theil-Sen) fits of FWHM vs. `guider_total_seeing` and vs. the detrended pointing sum-of-squares → FWHM-equivalents and signed quadrature residuals `FWHM² − FWHM_equiv²`; 2-D histograms of those relations and of each residual vs. `aos_fwhm²`; and residual histograms split into `N_AOS_QUANTILES` quantiles of `aos_fwhm`. Selectable `img_type`, guider quality cuts (`n_tracked_stars >= MIN_TRACKED_STARS`), coverage-based axis limits (`AXIS_COVERAGE`, default 0.99). | 2026-07-13 | 2026-07-13 |
